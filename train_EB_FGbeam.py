...
from train_utils.train_1d import train_1d_EB_FGbeam
from train_utils.losses import LpLoss, FDM_ReducedOrder2_Euler_Bernoulli_FGBeam_BSF_norm
...
def test(config, device):
    ...
    dx = 1 / (s - 1)
    x_test = data_loader.dataset[0][:, -1].repeat(batchsize).reshape(batchsize, s).to(device)
    bc = shape_function(data_config['BC'], x_test, 1.0).to(device)
    with torch.no_grad():
    ...
    ckpt_dir = 'checkpoints/%s/' % path
    # savemat(ckpt_dir + 'pifnoEB_bi_1025_rng1_powerlaw_height_linear_data5_r06r06f05_test.mat', {'x': test_x, 'yp': preds_y})
    # err_idx = np.argsort(test_err)
    err_idx = np.arange(0, nsample)
    np.random.shuffle(err_idx)
    plot_pred(data_config, test_x, preds_y, err_idx)
